  ...
  useEffect(() => {
        ...
        supabase.auth.getSession()
          .then(({ data: { session } }) => {
                    ...
                    return listAssets(orgId, kind || undefined, accessToken)
          })
          .then((res) => {
                    if (!mounted) return
  ...
  return (
        <div className="space-y-4">
          ...
          {loading ? (
                  ...
                  <p className="rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>p>
                ) : assets.length === 0 ? (
                                ...
                                <li key={a.id} className="overflow-hidden rounded-lg border border-gray-200 bg-white">
                                  {a.signed_url ? (
                                              ...
                                              <div className="border-t border-gray-200 p-2 text-xs">
                                                              <p className="font-medium text-gray-900">{a.kind}</p>p>
                                                {a.duration_seconds && <p className="text-gray-500">{Number(a.duration_seconds).toFixed(1)}s</p>p>}
                                              </div>div>
                                </li>li>
                              ))}
                  </ul>ul>
